...
export default function EditCategoryModal({ isOpen, onClose, category, onSuccess }) {
  ...
  useEffect(() => {
    if (category) {
      ...
      setBudgetType(category.budget_type || 'libre');
      
      const foundColor = COLORS.find(c => c.id === category.color);
      setSelectedColor(foundColor || COLORS[0]);
    }
  }, [category]);
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div className="w-full max-w-md bg-[#F0F2F5] rounded-3xl p-8 relative">
        ...
        <h2 className="text-2xl font-black text-slate-800 mb-6 tracking-tight">Editar Categoría</h2>
