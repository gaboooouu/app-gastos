...
export default function TransactionModal({ isOpen, onClose, accounts, onSuccess, onOpenNewCategory, categoryUpdateHash }) {
  ...
  // Filtrar solo las cuentas manuales
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Intentar preseleccionar la primera cuenta manual si no hay una seleccionada
      if (!accountId && manualAccounts.length > 0) {
        setAccountId(manualAccounts[0].id.toString());
      }
    }
  }, [isOpen, accounts, categoryUpdateHash]);

  // Si se cierra o no hay cuentas manuales? El render dejará que el usuario vea un mensaje
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    console.log('Enviando transacción:', { account_id: parseInt(accountId), amount: parseFloat(amount), description, budgetMonth });
    try {
      ...
      toast.success(type === 'gasto' ? 'Gasto registrado con éxito' : 'Ingreso registrado con éxito');
      setAmount('');
      setDescription('');
      setCategoryId('');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div className="w-full max-w-md bg-[#F0F2F5] rounded-3xl p-8 relative max-h-[95vh] overflow-y-auto custom-scrollbar">
        ...
        <h2 className="text-2xl font-black text-slate-800 mb-6 tracking-tight">Agregar Movimiento</h2>

        <div className="bg-slate-200/50 p-1.5 rounded-xl flex mb-6">
          <button
        ...
        {manualAccounts.length === 0 ? (
          <div className="text-center p-6 bg-orange-100 text-orange-800 rounded-xl mb-6 font-medium text-sm">
            No tienes cuentas manuales registradas. Para registrar gastos manualmente, primero debes Añadir una Cuenta Manual en el Dashboard.
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className={`neu-button mt-4 p-[10px] rounded-xl font-bold flex items-center justify-center gap-2 transition-colors ${
                type === 'gasto' ? 'bg-red-500 hover:bg-red-600 text-white shadow-red-500/20 shadow-lg' : 'bg-primary hover:bg-primary-dark text-slate-900 shadow-primary/20 shadow-lg'
              }`}
            >
              {loading ? (
                ...
                <span className="material-symbols-outlined text-sm">
                  {type === 'gasto' ? 'remove' : 'add'}
                </span>
              )}
              {loading ? 'Guardando...' : `Guardar ${type === 'gasto' ? 'Gasto' : 'Ingreso'}`}
            </button>
          </form>
        )}
      </div>
